...
//    tracerYText->position->setTypeX(QCPItemPosition::ptAxisRectRatio);
//    tracerYText->position->setTypeY(QCPItemPosition::ptPlotCoords);
//    tracerYText->position->setParentAnchor(0);
//    tracerYText->position->setParentAnchorY(tracer->position->parentAnchorY());

//    tracerArrow = new QCPItemLine(this);
//    tracerArrow->setHead(QCPLineEnding::esSpikeArrow);
//    tracerArrow->start->setParentAnchor(tracerYText->left);
...
//    qDebug() << "mouseMoveEvent";
    ...
    {
        if(getGraphs().contains(traceGraph))
        {
            double x = xAxis->pixelToCoord(event->pos().x());//像素坐标转plot坐标
